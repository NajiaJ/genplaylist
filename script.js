...
// of failing the whole generation over one bad request. Supports
// GET (default) and POST via options.method/options.body.
async function spotifyFetch(path, options = {}, attempt = 1) {
    let res;
    try {
...
async function createSpotifyPlaylist(userId, playlistName) {
    const body = {
        ...
        public: false,
        collaborative: false,
        description: `Generated by GenPlaylist • ${selectedIntent}`
    };

    // POST /me/playlists creates a playlist for the authenticated
    // user directly — no need to look up userId for the URL.
...
// =====================================================
// Spotify accepts up to 100 track URIs per request, so larger
// generated playlists are automatically split into batches.
